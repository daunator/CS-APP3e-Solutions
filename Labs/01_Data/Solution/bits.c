...
int fitsBits(int x, int n) {
  int shiftCount = ~n + 33;
  return !(x ^ (x << shiftCount >> shiftCount));
}
/* 
...
int isLessOrEqual(int x, int y) {
  int x_neg = x >> 31;
  int y_neg = y >> 31;
  int x_minus_y_min_1_neg = (x + ~y) >> 31;
  return 1 & ((x_neg & !y_neg) | ((!(x_neg ^ y_neg)) & x_minus_y_min_1_neg));
}
/*
...
int ilog2(int x) {
  int ms_x = x >> 16;
  int ms_mask = (!!ms_x << 31) >> 31;	
  int res = ms_mask & 16;

  x = (ms_mask & ms_x) | (~ms_mask & x);
  ms_x = x >> 8;
  ms_mask = (!!ms_x << 31) >> 31;
  res = res + (ms_mask & 8);
